    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container == True and place.ant.ant_contained != None:
                ...
                place.ant = self
            elif place.ant.is_container == True and self.is_container == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        i = 1
        while next_place.is_hive != True:
            if next_place.bees != [] and self.min_range <= i <= self.max_range:
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while last_place is not None:
            if last_place.ant is not None:
                last_place.ant.double()
                if last_place.ant.is_container and last_place.ant.ant_contained is not None:
                    last_place.ant.ant_contained.double()
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.scare_length > 0:
            if self.slowed > 0 and gamestate.time % 2 != 0:
                destination = self.place
                self.slowed -= 1
            elif self.slowed > 0 and gamestate.time % 2 == 0:
